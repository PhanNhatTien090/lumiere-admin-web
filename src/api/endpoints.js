...
export const orderAPI = {
    ...
    getInvoiceJson: (id) => axiosInstance.get(`/payments/orders/${id}/invoice`),
    confirmOrder: (id) => axiosInstance.put(`/orders/${id}/confirm`),
    cancelOrder: (id, reason) => axiosInstance.put(`/orders/${id}/cancel`, {
        reason,
    }),
...
export const categoryAPI = {
    list: () => axiosInstance.get("/manager/menu/categories"),
    getOne: (id) => axiosInstance.get(`/manager/menu/categories/${id}`),
    create: (data) => axiosInstance.post("/manager/menu/categories", data),
    update: (id, data) => axiosInstance.put(`/manager/menu/categories/${id}`, data),
    remove: (id) => axiosInstance.delete(`/manager/menu/categories/${id}`),
};
// ─── Menu Items (MANAGER) ───────────────────────────────────────────────────────
export const menuItemAPI = {
    /** GET /menu/items — requires categoryId (WAITER, MANAGER) */
    list: (categoryId) => axiosInstance.get("/menu/items", {
        params: { categoryId },
    }),
    create: (data) => axiosInstance.post("/manager/menu/items", data),
    update: (id, data) => axiosInstance.put(`/manager/menu/items/${id}`, data),
    remove: (id) => axiosInstance.delete(`/manager/menu/items/${id}`),
    getDetail: (id) => axiosInstance.get(`/manager/menu/items/${id}`),
    upsertFixedCombo: (id, data) => axiosInstance.put(`/manager/menu/items/${id}/combo/fixed`, data),
    upsertPickCombo: (id, data) => axiosInstance.put(`/manager/menu/items/${id}/combo/pick`, data),
    uploadImage: (id, file) => {
...
export const tableAPI = {
    ...
    create: (data) => axiosInstance.post("/manager/tables", data),
    update: (tableCode, data) => axiosInstance.put(`/manager/tables/${tableCode}`, data),
    remove: (tableCode) => axiosInstance.delete(`/manager/tables/${tableCode}`),
...
export const inventoryAPI = {
    listItems: () => axiosInstance
        .get("/manager/inventory/ingredients")
        .then((res) => ({
        ...res,
        data: {
            ...res.data,
    ...
    createItem: (data) => axiosInstance
        .post("/manager/inventory/ingredients", {
        name: data.name,
    ...
    updateItem: (id, data) => axiosInstance
        .put(`/manager/inventory/ingredients/${id}`, {
        name: data.name,
        unit: mapUnitToBackend(data.unit),
        lowStockThreshold: Number(data.minStock ?? 0),
    })
    ...
    removeItem: (id) => axiosInstance.delete(`/manager/inventory/ingredients/${id}`),
    listTransactions: async () => {
        ...
        const historyResponses = await Promise.all(items.map((item) => axiosInstance
            .get(`/manager/inventory/stock/${item.id}/history`)
    ...
    importStock: (data) => axiosInstance.post("/manager/inventory/stock/import", {
        ingredientId: data.itemId,
        quantity: data.quantity,
        note: data.note,
    }),
    exportStock: async (data) => {
        const itemRes = await axiosInstance.get(`/manager/inventory/ingredients/${data.itemId}`);
...
export const shiftAPI = {
    ...
    getSummary: (id) => axiosInstance.get(`/shifts/${id}/summary`),
...
export const aiAPI = {
    ...
    generateCombos: (data) => axiosInstance
        .post("/manager/menu/items/combo-generate", data)
        .then((res) => ({
        ...res,
        data: {
            ...res.data,
            data: {
                ...res.data.data,
                draftCombos: (res.data.data?.draft_combos || []).map((c) => ({
    ...
    triggerRetrain: () => axiosInstance.post("/manager/ai/retrain"),
    /** GET /manager/ai/jobs/{jobId} — poll retrain job status */
    pollRetrainJob: (jobId) => axiosInstance.get(`/manager/ai/jobs/${jobId}`),
};
// ─── Tax Config (MANAGER) ───────────────────────────────────────────────────────
export const taxAPI = {
    /** GET /manager/tax/config — lấy cấu hình thuế toàn cục */
    getConfig: () => axiosInstance.get("/manager/tax/config"),
    /** PUT /manager/tax/config — cập nhật cấu hình thuế */
    updateConfig: (data) => axiosInstance.put("/manager/tax/config", data),
    /** GET /manager/tax/preview/menu-items — xem trước breakdown gross/net/tax */
    previewMenuItems: () => axiosInstance.get("/manager/tax/preview/menu-items"),
};
